...
Transform.prototype.forward = function forward(path) {
  ...
  return transformations
    ...
    .map(function (t) {
      return {
        input: join(self.source, t.input),
        transformation: t.transformation,
        output: join(self.destination, t.output)
      }
    });
};
...
function render(transformations, cb) {
  parallelMap(transformations, function (route, cb) {
    fs.readFile(route.input, function (err, text) {
...
function toPathMutator(start, end) {
  ...
  } else if (Array.isArray(start) && start.length === 2 && start[0] instanceof RegExp &&
            (typeof start[1] === 'string' || typeof start[1] === 'function')) {
    return function (input) {
      if (start[0].test(input)) {
        return input.replace(start[0], start[1]);
      } else {
        return null;
      }
    }
